class ProjectAssignmentsController < ApplicationController
  ...
  def index
    if params[:engineer_id].nil? 
    @project_assignments = ProjectAssignment.paginate :all, :per_page => 10, :page => params[:page]
    else
    eid = params[:engineer_id]
    @project_assignments = ProjectAssignment.paginate :all, :conditions => "engineer_id = '#{eid}'",  :per_page => 10, :page => params[:page]
